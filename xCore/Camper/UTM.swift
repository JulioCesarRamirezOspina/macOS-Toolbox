...
//  SuperStuff
//
//  Created by Олег Сазонов on 26.05.2022.
//
...
//MARK: - UTM
//MARK: Public
/// Managing and launching UTM VMs
public class UTM: xCore {
    //MARK: - Functions
    //MARK: Private
    ...
    /// Gets list of UTM virtual machines in given directory
    /// - Returns: Array of addresses of VMs
    public class func getVMList() -> [URL] {
        var contents = [URL(fileURLWithPath: "")]
        var folderExists = false
        let pFolder = SettingsMonitor.utmDir
        do {
            contents = try FileManager.default.contentsOfDirectory(at: pFolder, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles, .skipsSubdirectoryDescendants])
        } catch _ {}
        var retval = [String()]
